...
function engine_renderWorld(size) {
  ...
  debug_newLogEntry(
    `Welcome to Dynamic Tile Map Generator v.${versionx}.`,
    0,
    0
  );
  ...
  if (Math.sqrt(size).toString().includes(".")) {
    if (size.toString().includes(".")) {
      debug_newLogEntry(
        `Invalid grid size '${size}': must not be a float number.`,
        1,
        3
      );
      return false;
      ...
      debug_newLogEntry(
        `Invalid grid size '${size}': not a square number.`,
        1,
        3
      );
      return false;
    ...
    debug_newLogEntry(
      `Invalid grid size '${size}': must not be null or negative.`,
      1,
      3
    );
    return false;
  ...
  if (usePerspective) {
    drawOverlay.style.transform =
      "rotateX(-55deg) rotateZ(-45deg) translate(27%, -97%)";
  } else {
  ...
  debug_newLogEntry(
    `Generated tile grid:\n    Grid size: ${gridSide}x${gridSide}\n    Tiles to render: ${gridSize}\n    Absolute tile size: ~${(
      grid.offsetWidth / gridSide
    ).toFixed(2)}px\n    Canvas resolution: ${quality * 32}x${
      quality * 32
    }px\n    Use perspective: ${usePerspective}`,
    1,
    0,
    1
  );
  ...
  debug_newLogEntry(
    `<details><summary>Textures selection probabilities</summary>${probabilities}</details>`,
    3,
    0
  );
  ...
  debug_newLogEntry(
    `Following cells will be omitted for left/right cell detection:\n    Left: ${nullLeftArray}\n    Right: ${nullRightArray}`,
    3,
    0
  );
  ...
  if (random <= 0.25) {
    ...
    debug_newLogEntry(
      `First reference tile set as ${gridSize - gridSide + 1}.`,
      1,
      0
    );
  } else if (random <= 1) {
  ...
  while (iterations < gridSize) {
    if (errorStatus === true) {
      ...
      debug_newLogEntry(
        `<hr><div style="text-align:center">Tile ${generatedCells + 1} (cell ${
          tileReference.id
        })</div><hr>`,
        1,
        -1
...
function gen_createReferenceTile(id) {
  var temp = {
    ...
    getTexture(tile) {
      ...
      if (tempTex === "undefined" || tempTex === "") {
        debug_newLogEntry(
          `Unset or undefined texture for cell #${tileReference.id}: ${tempTex}`,
          3,
          1
        );
        tempTex = "void";
    ...
    getAdjacentTextures() {
      ...
      debug_newLogEntry(
        `Adjacent textures: ${top} ${bottom} ${left} ${right}.`,
        3,
        0
      );
    ...
    getAdjacentTexturesCategories() {
      ...
      debug_newLogEntry(
        `Adjacent textures <em>categories</em>: ${top} ${bottom} ${left} ${right}.`,
        3,
        0
      );
      return top + bottom + left + right;
    ...
    setTexture(src) {
      if (tileReference.master) {
        ...
        debug_newLogEntry(
          `Applying texture '${src}.png':\n    X offset: ~${a.toFixed(
            2
          )} px\n    Y offset: ~${b.toFixed(2)} px`,
          3,
          0
        );
      }
  ...
  if (temp.left) {
    if (nullLeftArray.includes(temp.left.id.slice(4))) {
      debug_newLogEntry(
        `Omitting #${temp.left.id} in compliance with <em>nullLeftArray</em>.`,
        3,
        0
      );
      temp.left = null;
  ...
  if (temp.right) {
    if (nullRightArray.includes(temp.right.id.slice(4))) {
      debug_newLogEntry(
        `Omitting #${temp.right.id} in compliance with <em>nullRightArray</em>.`,
        3,
        0
      );
      temp.right = null;
...
function matchTextures() {
  ...
  debug_newLogEntry(
    `<details><summary>Current textures set for probabilities</summary>${posArray}</details>`,
    3,
    0
  );
...
function randomTexture(textures) {
  ...
  let probabilities = [...probabilitiesSet].sort();
...
function createNewTile() {
  ...
  if (adjacentOutput === "voidvoidvoidvoid") {
    debug_newLogEntry(
      `Detected empty map, generating first texture randomly.`,
      2,
      0
    );
    ...
    debug_newLogEntry(
      `Detected non-empty map, matching available textures.`,
      3,
      0
    );
  ...
  if (engine_singleTileMap) {
    ...
    debug_newLogEntry(
      `Using single texture map: ${engine_singleTileMapTex}`,
      1,
      "DevMode"
    );
  }
  ...
  switch (tileReference.getAdjacentTexturesCategories()) {
    ...
    default:
      debug_newLogEntry(
        `Unknown textures categories combination "${tileReference.getAdjacentTexturesCategories()}"`,
        1,
        1
      );
      break;
  ...
  debug_newLogEntry(
    `Applied texture "${texture}" to cell #${oldTileReference.id}`,
    2,
    0
  );
  if (gridSize - generatedCells === 0) {
    ...
    debug_newLogEntry(
      `Tiles to render left: ${gridSize - generatedCells}`,
      2,
      0
    );
  } else {
